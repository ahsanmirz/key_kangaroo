...
void Kangaroo::CreateHerd(int nbKangaroo,Int *px,Int *py,Int *d,int firstType,bool lock) {
  ...
  // Choose structured-random starting distance (stratified sampling across the range).
  // This spreads kangaroos more evenly than fully-random picks while keeping per-kangaroo randomness.
  Int tameWidth;
  Int wildShift;
#ifdef USE_SYMMETRY
  // Tame in [0..N/2], Wild in [-N/4..N/4]
  tameWidth.Set(&rangeWidthDiv2);
  wildShift.Set(&rangeWidthDiv4);
#else
  // Tame in [0..N], Wild in [-N/2..N/2]
  tameWidth.Set(&rangeWidth);
  wildShift.Set(&rangeWidthDiv2);
#endif

  Int step(&tameWidth);
  Int rem;
  Int div((uint64_t)nbKangaroo);
  step.Div(&div, &rem); // step = tameWidth / nbKangaroo
  if(step.IsZero())
    step.SetInt32(1);
