...
// Portable fallback (ARM64 / Apple Silicon, etc.). Slower than x86 asm/intrinsics, but correct.
static uint64_t inline _umul128(uint64_t a, uint64_t b, uint64_t *h) {
...
static unsigned char inline _addcarry_u64(unsigned char c, uint64_t a, uint64_t b, uint64_t *out) {
  unsigned __int128 s = (unsigned __int128)a + (unsigned __int128)b + (unsigned __int128)c;
  *out = (uint64_t)s;
  return (unsigned char)((s >> 64) & 0x1);
}

static unsigned char inline _subborrow_u64(unsigned char c, uint64_t a, uint64_t b, uint64_t *out) {
  unsigned __int128 bb = (unsigned __int128)b + (unsigned __int128)c;
  unsigned char borrow = ((unsigned __int128)a < bb) ? 1 : 0;
  *out = (uint64_t)((unsigned __int128)a - bb);
  return borrow;
}
